...
export const Route = createFileRoute("/notes")({
  ...
  loader: async ({ deps, context }) => {
    ...
    await qc.ensureQueryData(notesListQuery(deps.page, deps.limit));
  },
...
function NotesError({ reset }: { error: Error; reset: () => void }) {
  const { t } = useTranslation();
  return (
    <div className="flex min-h-svh items-center justify-center p-6 bg-muted/20">
      <Card className="max-w-md w-full">
        <CardHeader>
          <CardTitle>{t("api.note.fetchFailed")}</CardTitle>
...
function NotesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: CreateNoteDto) => {
      ...
      const res = await client.notes.create({ body: data });
      if (res.status !== 201) throw new Error("api.note.createFailed");
      return res.body;
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      if (res.status !== 204) throw new Error("api.note.deleteFailed");
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["notes"] });
        ...
        <div className="container mx-auto flex h-14 items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            <Badge variant="outline">
              {t("common.total")}: {notesQuery.data?.total ?? "—"}
            </Badge>
            ...
            <CardContent className="space-y-4">
              ...
              {notesQuery.data && notesQuery.data.totalPages > 1 && (
                <DataTablePagination
                  ...
                  pageLabel={(currentPage, pages) =>
                    t("common.pageOf", { page: currentPage, totalPages: pages })
                  }
