...
export const Route = createFileRoute("/dashboard")({
  ...
  loader: async ({ context }) => {
    ...
    await qc.ensureQueryData(notesListQuery(1, 5));
  },
  ...
  return (
    <div className="flex min-h-svh items-center justify-center p-6 bg-muted/20">
      <Card className="max-w-md w-full">
        <CardHeader>
          <CardTitle>{t("dashboard.loadFailed")}</CardTitle>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardDescription className="flex items-center gap-2">
                <FileText className="size-4" /> {t("dashboard.stats.notes")}
              </CardDescription>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-xs text-muted-foreground">
                {t("tenancy.createOrganizationDescription")}
              </p>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent>
              ...
              {notesQuery.isError && (
                <p className="text-sm text-destructive">
                  {t("notes.loading")}: {t("errors.networkError")}
                </p>
              )}
              {notesQuery.data && (
