...
export abstract class BaseRepository<TEntity, TRow> extends BaseReadRepository<TEntity, TRow> {
  async create(data: Record<string, unknown>): Promise<Result<TEntity, never>> {
    if (this.hasMissingTenantContext()) {
      throw new Error("TENANT_REQUIRED: Cannot insert tenant-scoped entity without active tenant context");
    }
    const db = this.getDb();
    const payload = { ...data, ...this.tenantFilter(), id: (data["id"] as string) ?? crypto.randomUUID(), createdAt: new Date(), updatedAt: new Date() } as unknown as TRow;
    const rows = await (db as unknown as { insert: (t: unknown) => { values: (v: unknown) => { returning: () => Promise<TRow[]> } } })
      .insert(this.table).values(payload as unknown as Record<string, unknown>).returning();
    return ok(this.toDomain(rows[0] as TRow));
  }

  async createMany(data: Record<string, unknown>[]): Promise<Result<TEntity[], never>> {
    if (this.hasMissingTenantContext()) {
      throw new Error("TENANT_REQUIRED: Cannot insert tenant-scoped entities without active tenant context");
    }
    const db = this.getDb();
    const payloads = data.map((d) => ({ ...d, ...this.tenantFilter(), id: (d["id"] as string) ?? crypto.randomUUID(), createdAt: new Date(), updatedAt: new Date() }));
    const rows = await (db as unknown as { insert: (t: unknown) => { values: (v: unknown) => { returning: () => Promise<TRow[]> } } })
      .insert(this.table).values(payloads as unknown as Record<string, unknown>[]).returning();
  ...
  async paginate(filter: Record<string, unknown> = {}, options: PaginationOptions = {}): Promise<Result<PaginatedResult<TEntity>, never>> {
    if (this.hasMissingTenantContext()) {
      return ok({ items: [], total: 0, page: options.page ?? 1, limit: options.limit ?? 20, totalPages: 1, hasNextPage: false, hasPrevPage: false });
    }
    const db = this.getDb();
    ...
    const [items, totalRes] = await Promise.all([
      (db as unknown as { select: () => { from: (t: unknown) => { where: (c: unknown) => { limit: (n: number) => { offset: (o: number) => Promise<TRow[]> } } } } }).select().from(this.table).where(conditions).limit(limit).offset(offset),
      (db as unknown as { select: (v: unknown) => { from: (t: unknown) => { where: (c: unknown) => Promise<{ count: number }[]> } } }).select({ count: sql<number>`count(*)` }).from(this.table).where(conditions),
    ]);
    const total = Number(totalRes[0]?.count ?? 0);
    const totalPages = Math.max(1, Math.ceil(total / limit));
    return ok({ items: items.map((r) => this.toDomain(r)), total, page, limit, totalPages, hasNextPage: page < totalPages, hasPrevPage: page > 1 });
  }
  ...
  async updateById(id: Id, update: Record<string, unknown>): Promise<Result<TEntity | null, { type: "CONFLICT" }>> {
    ...
    const tenantClause = tenantFilter && (this.table as unknown as Record<string, unknown>)["tenantId"]
      ? eq((this.table as unknown as Record<string, unknown>)["tenantId"] as Parameters<typeof eq>[0], tenantFilter["tenantId"] as string) : undefined;
    ...
    const rows = await (db as unknown as { update: (t: unknown) => { set: (v: unknown) => { where: (c: unknown) => { returning: () => Promise<TRow[]> } } } })
      .update(this.table).set({ ...update, updatedAt: new Date() } as unknown as Record<string, unknown>).where(whereClause).returning();
    const row = rows[0] ?? null;
    return ok(row && !(row as unknown as Record<string, unknown>)["deletedAt"] ? this.toDomain(row) : null);
  }
  ...
  async updateOne(filter: Record<string, unknown>, update: Record<string, unknown>): Promise<Result<TEntity | null, { type: "CONFLICT" }>> {
    ...
    const conditions = this.buildConditions({ ...filter, ...this.tenantFilter() });
    const rows = await (db as unknown as { update: (t: unknown) => { set: (v: unknown) => { where: (c: unknown) => { returning: () => Promise<TRow[]> } } } })
      .update(this.table).set({ ...update, updatedAt: new Date() } as unknown as Record<string, unknown>).where(conditions as never).returning();
  ...
  async softDeleteById(id: Id): Promise<Result<TEntity | null, never>> {
    ...
    const whereClause = tenantClause ? and(eq(idCol, id as string), tenantClause, deletedClause) : and(eq(idCol, id as string), deletedClause);
    const rows = await (db as unknown as { update: (t: unknown) => { set: (v: unknown) => { where: (c: unknown) => { returning: () => Promise<TRow[]> } } } })
      .update(this.table).set({ deletedAt: new Date(), updatedAt: new Date() } as unknown as Record<string, unknown>).where(whereClause).returning();
    return ok(rows[0] ? this.toDomain(rows[0]) : null);
  }
